	...
	class YITH_WooCommerce_Gift_Cards_Premium extends YITH_WooCommerce_Gift_Cards {
		...
		public function includes() {
			...
			if ( defined( 'WOOCOMMERCE_MULTICURRENCY_VERSION' ) ){
				require_once ( YITH_YWGC_DIR . 'lib/third-party/class-ywgc-WCMultiCurrency-module.php' );
			}
...
//			add_filter ( 'yith_gift_cards_before_add_to_cart', array( $this, 'customize_card_before_add_to_cart' ) );
		...
		public function action_links( $links ) {
			...
			$links = yith_add_action_links( $links, $this->_panel_page, true, YITH_YWGC_SLUG );
		...
		public function init_metabox() {
			// get all bookable product(event) for Enable for those Event
			$event_args = array('numberposts' => -1,'type' => 'booking', 'post_status' => 'publish');
			
			$events = wc_get_products($event_args);
			
			$events = array_column($events, 'name','id'); // keep only 2 column
			// echo '<br/>';
			// print_r($events);
			// get all bookable product(event)
			$args = array(
					...
					'General' => array( //tab
						...
						'fields' => apply_filters ( 'yith_ywgc_gift_card_instance_metabox_custom_fields',
							array(
								...
								'_ywgc_internal_event_id' =>array(
									'label'   => esc_html__( 'Enable for these event', 'yith-woocommerce-gift-cards' ),
									'desc'    => esc_html__( 'Hold ctrl to select multiple'),
									'type'    => 'select',
									'options' => array(
									''       => 'All Events')+$events, // show this array in 
		...
		public function check_gift_card( $gift, $remove = false ) {
			...
			elseif ( WC()->cart->total < $gift->get_balance() ) {
				wc_add_notice( esc_html__( 'Please purchase event equal to or more than the amount given in your Gift Card', 'yith-woocommerce-gift-cards'), 'error' );
		...
		public function ywgc_display_product_addons( $object ){
			...
			if ( is_object( $order ) ) {
					...
					foreach ( $order->get_items() as $item_id => $item ) {
						...
						$item_gc_number  = wc_get_order_item_meta( $item_id, '_ywgc_gift_card_code', true );

						if ( is_array( $item_gc_number ) && ! in_array( $object->gift_card_number, $item_gc_number, true ) ) {
							continue;
						}
						...
						if ( is_array( $addons_array ) && !empty( $addons_array ) /*|| isset( $addons_array ) */ ) {

							foreach ( $addons_array as $addon ) {

								if ( isset( $addon['name'] ) && isset( $addon['value'] ) ){
									?><tr><td colspan="2" ><?php echo $addon['name'] . ': ' . $addon['value']; ?></td></tr><?php
								}
							}
						}
					}
		...
		public function ywgc_count_user_gift_cards( $user_id ){
			...
			$user_gift_cards_number = count($ids);

			return $user_gift_cards_number;
		}
