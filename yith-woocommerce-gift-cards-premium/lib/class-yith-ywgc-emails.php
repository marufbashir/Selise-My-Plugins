...
if ( ! class_exists ( 'YITH_YWGC_Emails' ) ) {
	...
	class YITH_YWGC_Emails {
		...
		public function send_delayed_gift_cards() {
			...
			$send_date = apply_filters( 'ywgc_send_delayed_gift_cards_send_date', strtotime( 'today 23:59:59' ) + ( get_option( 'gmt_offset' ) * 3600 ) );
...
//to send the event available events ----------------------------------------------------------------@@@@@@@@@@@@@@@@@@@@

$giftcardID= $gift_card->ID;
			
$eventsIds = get_post_meta($giftcardID, '_ywgc_internal_event_id', TRUE);
...
if(!empty($eventsIds)){
	...
	//$availableProducts = get_posts($eventArgs);
	$loop = new WP_Query( $eventArgs ); 
	if(!empty($loop) && $loop->post_count > 0){
		echo '<p><strong>';
		_e('This Gift Card Only Available for These Events','astra');
		echo '</strong></p>';
		echo '<ul class="available-event-list">'; ?>
		<?php while ( $loop->have_posts() ) : $loop->the_post(); ?>
			<li class="available-event-item" style="line-height: 35px;">
...
// end send the event available events---------------------------------------------------------------- @@@@@@@@@@@@@@@@@
