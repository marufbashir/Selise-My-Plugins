...
if ( ! class_exists( 'YITH_YWGC_Cart_Checkout' ) ) {
	...
	class YITH_YWGC_Cart_Checkout {
		...
		public function init_hooks() {
			...
			add_action( 'wp_ajax_no_priv_edit_gift_card', array( $this, 'edit_gift_card_callback' ) );
			...
			if ( apply_filters( 'ywgc_add_item_fee_for_gift_cards', false ) ){
				add_action( 'woocommerce_checkout_order_processed', array($this,'add_item_fee_for_gift_cards'), 99 );
			}
...
//							$cart->set_subtotal_tax($new_tax_value);
		...
		public function apply_gift_cards_discount( $cart ) {
			...
			if ( count($gift_card_codes)>1 ) {
				
				if (is_checkout()) {
			wc_add_notice( esc_html__( "Sorry! Only one code can be used per event.", 'yith-woocommerce-gift-cards'), 'notice' );
			}
			}
			if ( $gift_card_codes ) {
		...
		public function apply_gift_card_code_callback() {
			...
			//echo 'Event ID', $product_id; // only get the last event id

			// new codes end
			if ( ! empty( $code ) ) {
				$gift = YITH_YWGC()->get_gift_card_by_code( $code );
				// echo'gift id ', $gift->ID;
				$valid_event_ids = get_post_meta($gift->ID, '_ywgc_internal_event_id', TRUE);
				//print_r($valid_event_ids);
				if($valid_event_ids){
					foreach($valid_event_ids as $validEvent){
						
						if($product_id == $validEvent){
							...
							$giftCodeIsValid = true;
							
						}
						else{
							$errorMessgaeNotForThisEvent = 'This gift code is not valid for this event.';
						}
					}
					...
					if($giftCodeIsValid){
						...
						echo '<p style="color: red;">',_e($errorMessgaeNotForThisEvent,'astra'),'</p>';
					}
		...
		public function register_gift_cards_usage( $order_id ) {
			...
			if ( $applied_gift_cards ) {
				...
				yit_save_prop( $order, '_ywgc_applied_gift_cards', $applied_gift_cards );
				yit_save_prop( $order, '_ywgc_applied_gift_cards_totals', $applied_discount );
				yit_save_prop( $order, '_ywgc_applied_gift_cards_order_total', $order_total );
		...
		public function add_item_fee($order_id) {
			...
			if ( $order->get_shipping_method() == '' || $order->get_shipping_total() == 0 || $order->get_shipping_total() == '0.00' ){ //If no shipping or if shipping is zero, dont add it as fee
				return;
			}
			...
			$total_coupon = $total_coupons_amount + $total_coupons_amount_tax;

			$shipping_total_value = $order->get_shipping_total() + $order->get_shipping_tax();
			$order_total = $order->get_total() + $total_coupon;

			if ( $total_coupon >! 0 ){
				return;
			}

			if ( ( $order_total - $shipping_total_value ) > $total_coupon ){ //If the shipping is not included in the coupon, dont add it as fee
				return;
			}

			update_post_meta( $order_id, 'ywgc_applied_coupons_array', $coupon_codes_array );

			if ( $total_coupons_amount > 0 ){

				$total_coupon_value = $total_coupons_amount + $total_coupons_amount_tax;

				update_post_meta( $order_id, 'ywgc_applied_coupons_as_fees_total', $total_coupon_value );

				foreach( $order->get_items( 'tax' ) as $item_id => $item_tax ){
					$tax_data = $item_tax->get_data();
					$tax_rate = $tax_data['rate_percent'];
					$tax_rate_id = $tax_data['rate_id'];
				}
				...
				$amount = round (-1 * ($total_coupon_value/(1 + $rate_aux) ), 2 );

				$aux_order_total = $order->get_total();
				...
				$aux_cart_tax = $order->get_cart_tax();
				...
				foreach( $order->get_items( 'fee' ) as $item_id => $item_fee ){
					$fee_total = wc_get_order_item_meta( $item_id, '_line_total', true );

					$line_tax_aux = -1 * ((float)$total_coupon_value + (float)$amount );

					$line_tax_data_aux = array(
						'total' => array( $tax_rate_id => $line_tax_aux ),
					);


					if ( $fee_total != $amount ){
						wc_update_order_item_meta( $item_id, '_line_total', $amount );
						wc_update_order_item_meta( $item_id, '_line_tax', $line_tax_aux );
						wc_update_order_item_meta( $item_id, '_line_tax_data', $line_tax_data_aux );
						wc_update_order_item_meta( $item_id, '_ywgc_aux_line_total', $amount );
						wc_update_order_item_meta( $item_id, '_ywgc_aux_line_tax', $line_tax_aux );
					}
				}

				update_post_meta($order->get_id(), '_ywgc_aux_cart_total_tax', $aux_cart_tax );
				update_post_meta($order->get_id(), '_order_tax', $aux_cart_tax );
				update_post_meta($order->get_id(), '_ywgc_aux_cart_total', $aux_order_total );
				...
				update_post_meta($order->get_id(), '_ywgc_applied_gift_cards_as_fees', true );
		...
		public function add_item_fee_for_gift_cards($order_id) {
		...
		public function show_gift_card_details_in_cart( $item_data, $cart_item ) {
			...
			if ( is_cart() || is_checkout() && isset(  $cart_item['ywgc_product_id'] ) ){
				...
				if ( isset(  $cart_item['ywgc_recipient_name'] ) && ! empty( $cart_item['ywgc_recipient_name'] ) ) {
					echo '<div style="margin-top: 5px">'. esc_html__( 'Recipient\'s name: ', 'yith-woocommerce-gift-cards' ) . strip_tags( preg_replace( '#<script(.*?)>(.*?)</script>#', '', $cart_item['ywgc_recipient_name'] ) ) .'</div>';
				}
				if ( isset(  $cart_item['ywgc_recipients'] ) && ! empty( $cart_item['ywgc_recipients'] ) ) {
					...
					echo '<div>'. esc_html__( 'Recipient\'s email: ', 'yith-woocommerce-gift-cards' ) . strip_tags( preg_replace( '#<script(.*?)>(.*?)</script>#', '', $value ) ) .'</div>';
				}

				if ( isset(  $cart_item['ywgc_sender_name'] ) && ! empty( $cart_item['ywgc_sender_name'] ) ) {
					echo '<div>'. esc_html__( 'Your name: ', 'yith-woocommerce-gift-cards' ) . strip_tags( preg_replace( '#<script(.*?)>(.*?)</script>#', '', $cart_item['ywgc_sender_name'] ) ) .'</div>';
				}

				if ( isset(  $cart_item['ywgc_message'] ) && ! empty( $cart_item['ywgc_message'] ) ) {
					echo '<div>'. esc_html__( 'Message: ', 'yith-woocommerce-gift-cards' ) . nl2br(stripslashes( strip_tags( preg_replace( '#<script(.*?)>(.*?)</script>#', '', $cart_item['ywgc_message'] ) ) ) )  .'</div>';
				}
		...
		public function build_cart_item_data() {
			...
			if ( $ywgc_is_digital ) {
				...
				if ( 'custom' == $design_type ) {
					...
					if ( isset( $_FILES["ywgc-upload-picture"] ) )  {
						...
						if ( empty( $custom_image['name'] ) ) {
							wc_add_notice( _( 'There was an error with the image file you uploaded; please, try again', 'upload image error message', 'yith-woocommerce-gift-cards' ), 'error' );
							return false;
						}
						...
						$max_size_byte = get_option ( 'ywgc_custom_image_max_size', 1 ) * 1024 * 1024;
				...
				$delivery_notification_enabled = isset( $_POST['ywgc-delivery-notification-checkbox'] ) ? $_POST['ywgc-delivery-notification-checkbox'] : 'off';

				if ( $delivery_notification_enabled == 'on' ){
			...
			if ( $product_as_present ) {
				...
				if ( isset( $_POST['ywgc-gift-this-product-total-value'] )
				     && isset( $_POST['ywgc-include-shipping-checkbox'] ) && $_POST['ywgc-include-shipping-checkbox'] == 'on'  ){

					$ywgc_amount = $_POST['ywgc-gift-this-product-total-value'];
				}
				else{
		...
		public function ywgc_modify_paypal_args( $args, $order ) {
			...
			$gift_cards_applied = yit_get_prop( $order, '_ywgc_applied_gift_cards', true );
			$gift_cards_applied_as_fees = yit_get_prop( $order, '_ywgc_applied_gift_cards_as_fees', true );
			...
			if ( ! $gift_cards_applied && ! $gift_cards_applied_as_fees && ! $total_applied_as_default_coupon > 0 ) {
				return $args;
			}
			...
			if ( $gift_cards_applied ){
				$discount_amount_cart = yit_get_prop( $order, '_ywgc_applied_gift_cards', true );
			}

			if ( $gift_cards_applied_as_fees ){
				$discount_amount_cart = yit_get_prop( $order, 'ywgc_applied_coupons_as_fees_total', true );
			}
		...
		public function ywgc_remove_order_again_button_gift_this_product( $order ) {
			...
			if ( in_array( 'gift-card', $product_types_array ) ) {
